...
左ツールバーで `Normal` を選び，編集したい頂点または辺をクリックします．

ポップアップから，頂点ラベル・辺ラベル・色・サイズなどを編集できます．
...
辺ラベルは `JSON` 形式で保存・復元できます．表示するには，辺の `style.text` を `"label"` にし，画面下部の `Text: Label` を有効にします．
...
|     `A`     | アニメーションを切り替える         |
|     `T`     | Text: ID / Label / Hidden を切り替える |
|     `[`     | グラフ全体を左回転する             |
|     `]`     | グラフ全体を右回転する             |

ラベル入力欄を編集中は，`A` と `T` のショートカットは無効になります．
